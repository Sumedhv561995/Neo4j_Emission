...
from llmrelationshiptest import target_join_column
from llmrelationshiptest1 import target_sheet_name
...
sap_hierarchy_file = config['excel_file']
wb_hierarchy = openpyxl.load_workbook(sap_hierarchy_file)

operating_parameters_file = config['additional_excel_file']
wb_operating_params = openpyxl.load_workbook(operating_parameters_file)

sheet_names = wb_operating_params.sheetnames
print("Sheet names:", sheet_names)

# Example to check for leading/trailing spaces in sheet names
sheet_names_cleaned = [sheet.strip() for sheet in sheet_names]
print("Cleaned sheet names:", sheet_names_cleaned)
...
for relationship in relationships:
    ...
    source_column = relationship['join_column']
    target_column = relationship['target_join_column']
    ...
    try:
        ...
        print(f"Error: Join column '{source_column}' not found in sheet '{sheet_name}'.")
        continue
        ...
        for target_row in target_sheet.iter_rows(min_row=2, values_only=True):
            if target_row[target_join_column_index] == join_column_value:
                relationship_data.append({
                    "query": f"""
                        MATCH (a:`{sheet_name}` {{ `{source_column}`: $join_column_value }})
                        MATCH (b:`{target_sheet_name}` {{ `{target_column}`: $target_join_column_value }})
                        MERGE (a)-[:{rel_type}]->(b)
                    """,
                    "parameters": {
                        "join_column_value": join_column_value,
                        "target_join_column_value": target_join_column_value
                    }
                })
